...
var customContainer = $('#overlay').append($(gui.domElement));
d3.select("#overlay")
    .style("transform", "translate(" + ($('#map').width() - 200) + "px,0px)");
...
// ==============flow-field
var flowFieldFolder = gui.addFolder("Epidemic Shift");
var flowConfig = {
    visible: true,
    opacity: 1
};

var flowFieldVis = flowFieldFolder.add(flowConfig, "visible");
flowFieldVis.onChange(function (value) {
   mapboxSvg.style("display", value ? "block": "none");
});

var flowOpacity = flowFieldFolder.add(flowConfig, 'opacity', 0, 1);
